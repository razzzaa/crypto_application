...
$(window).on("load", function () {
    ...
    function liveGraph(toggledCoinsData) {
        ...
        function updateChart(count) {
            ...
            $.ajax({
            method: 'GET',
            headers: {'authorization': '521c5383500d5183ced0dab62d389bc2679ff66aff6ccce5152288acb3644a85' },
            ...
            success: function(oi) {
                ...
                while(counts < stopper) {
                    chart.options.data[counts].legendText = `${toggledCoinsData[counts].cardSymbol} : $` + usdValue[counts];
                    counts++
                }
                chart.render();
                }
    ...
    function rndmCryptoCall(fullCrptlist) {
        ...
        lessThanhndrd.forEach((coins, index) => {
            ...
            success: function (coinsInfo) {
                ...
                <h3>ILS: &#8362; ${
                coinsInfo.market_data.current_price
                .ils
                }</h>
                <h3>USD: &#36; ${
                coinsInfo.market_data.current_price
            ...
            complete: function() {
                if (numFailures === numAjaxCalls) {
                    $('#mainCardsDiv').append(`<iframe id='ytVid' width="560" height="560" src="https://www.youtube.com/embed/__bNjF-xR1U"
                    title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`
                    );
                    $('#fuckingCoinGeckoLimitationsNshit').modal('show');
                    setTimeout(() => {rndmCryptoCall(fullCrptlist)}, 120000) 
                }
            }
            });
        });
    ...
    $("#contctBtn").on("click", function () {
        ...
        newShortList.forEach((crypt, index) => {
            ...
            $("#fullList").append(`
            <div class="card text-bg-dark rndCoinsCard" data-id='${index + 201}'>
            <div class="toggleImage">
    ...
    function deleteCardFromToggledCards(cardIdd) {
        const index = toggledCards.findIndex(
            (card) => card.cardId === cardIdd
        );
    ...
    $("#liveList").on("click", ".delete-btn", function () {
        ...
        if(toggledCards.length === 0){
            setTimeout(() => {$('#emptyPop').modal('show');
            setTimeout(() => { $('#contctBtn').click()}, 2000);
            }, 500);
        }
    });
